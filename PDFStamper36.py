        ...
        for j in range(len(codes['Pack ID'])):
            if (userlist.loc[i, 'Document']) in (codes.loc[j, 'Pack ID']):
                ...
                if (codes.loc[j, 'Multiple']) == 'M':
                    ...
                    msg = 'Requested code ' + multiplecodename + ' contains ' + multiplecodebmr + ' which uses MULTIPLE batch numbers and it is not yet supported. Reemove from the request list.'
                    error_handling(msg)
