"""
PDFStamper v4.2  - Codename: Aggie
...
# Password control
def passwordcontrol(username, msg):
    systemname = "PDFStamper"
    from easygui import passwordbox
    userpassword = passwordbox(msg)
...
print('Welcome to PDFStamper 4.2 release - Codename: Aggie.')
print('\n')
import wx
...
print('You are running version 4.2. Checking for a new version...', end=' ')
import csv
...
try:
    ...
    if (float(new_version[0]) > float(current_version)):
        ...
        if reply == "Update Now":
            ...
            import urllib
            download = 'PDFStamper_Setup.exe'
            from tqdm import tqdm
            r = urllib.request(new_version[1])
...
if update_flag == 1:
    subprocess.Popen(download)
    raise SystemExit

# Handle password to access BTG Proquis and BSC SharePoint
...
# BSC password (bscusername is retrieved from Windows)
bscusername = getuser()
bscuserpassword = keyring.get_password("PDFStamper", bscusername)
if (bscuserpassword is None):
    msg = 'Please, enter your BSCi password to access SharePoint: '
    bscuserpassword = passwordcontrol(bscusername, msg)
# BTG username and password
import pickle
try:
    f = open('btgusername.pkl', 'rb')
    btgusername = pickle.load(f)
    f.close
except:
    f = open('btgusername.pkl', 'wb')
    import easygui
    from easygui import enterbox
    windowtitle = 'BTG network access'
    msg = 'Please enter your BTG username to access Proquis (e.g. asouthall): '
    btgusername = enterbox(msg, windowtitle)
    pickle.dump(btgusername, f)
    f.close()
btguserpassword = keyring.get_password("PDFStamper", btgusername)
if (btguserpassword is None):
    msg = 'Please enter your BTG password to access BTG Proquis: '
    btguserpassword = passwordcontrol(btgusername, msg)
...
try:
    download_file(codesurl, outputfile, btgusername, btguserpassword)
except Exception:
...
for x in tqdm(range(len(downloadlist)), unit='file'):
    if 'FAR-' in str(downloadlist[x]):
        ...
        download_file(urlfile, outputfile, btgusername, btguserpassword)
    elif 'SI-' in str(downloadlist[x]):
        tqdm.write('Downloading: ' + downloadlist[x])
        try:
            si_partialname = str(downloadlist[x])
            si_filename = download_si(si_partialname, btgusername, btguserpassword)
        except Exception:
